...
var require_utils = __commonJS({
  "src/utils.js"(exports2, module2) {
    ...
    async function runCmd(cmd, options = {}) {
      ...
      const patchedPath = process.platform === "win32" || envPath.includes("/usr/sbin") ? envPath : `${systemPath}:${envPath}`;
      const opts = {
        encoding: "utf8",
        timeout: 1e4,
        env: {
...
var require_config = __commonJS({
  "src/config.js"(exports2, module2) {
    ...
    function expandPath(p) {
      if (!p) return p;
      const expanded = p.replace(/^~/, HOME).replace(/\$HOME/g, HOME).replace(/\$\{HOME\}/g, HOME);
      return expanded === p ? p : path2.normalize(expanded);
    }
    function loadConfig() {
      ...
      const config = {
        ...
        auth: {
          ...
          trustProxy: (process.env.DASHBOARD_TRUST_PROXY || "").toLowerCase() === "true" || fileConfig.auth?.trustProxy === true
        },
        // Branding
        branding: {
        ...
        billing: {
          ...
          tokenRates: fileConfig.billing?.tokenRates || void 0
        }
      };
      return config;
    }
...
var require_vitals = __commonJS({
  "src/vitals.js"(exports2, module2) {
    ...
    async function collectWindowsVitals(vitals) {
      ...
      const usage = totalDelta > 0 ? Math.max(0, Math.min(100, Math.round(100 * (1 - idleDelta / totalDelta)))) : 0;
      ...
      try {
        ...
        const script = "Get-CimInstance Win32_LogicalDisk | Select-Object DeviceID,Size,FreeSpace | ConvertTo-Json -Compress";
        ...
        if (raw) {
          ...
          const drive = drives.find((d) => (d.DeviceID || "").toUpperCase() === sysDrive.toUpperCase()) || drives.find((d) => Number(d.Size) > 0);
    ...
    var vitalsRefreshInterval = null;
    function startVitalsRefresh2() {
    ...
    module2.exports = {
      refreshVitalsAsync,
      startVitalsRefresh: startVitalsRefresh2,
...
var require_auth = __commonJS({
  "src/auth.js"(exports2, module2) {
    ...
    function checkAuth2(req, authConfig) {
      ...
      if (mode === "allowlist") {
        const forwardedFor = authConfig.trustProxy ? req.headers["x-forwarded-for"]?.split(",")[0]?.trim() : null;
...
var require_operators = __commonJS({
  "src/operators.js"(exports2, module2) {
    ...
    var opCache = { dataDir: null, data: null, timestamp: 0 };
    var OP_CACHE_TTL = 5e3;
    ...
    function startOperatorsRefresh2(dataDir, getOpenClawDir2) {
      setTimeout(() => refreshOperatorsAsync(dataDir, getOpenClawDir2), 2e3).unref?.();
      setInterval(() => refreshOperatorsAsync(dataDir, getOpenClawDir2), 5 * 60 * 1e3).unref?.();
    }
...
var require_sessions = __commonJS({
  "src/sessions.js"(exports2, module2) {
    ...
    var CHANNEL_MAP = DEFAULT_CHANNEL_MAP;
    function loadChannelMap2(dataDir) {
      const map = { ...DEFAULT_CHANNEL_MAP };
      if (!dataDir) return map;
      try {
        const file = path2.join(dataDir, "channels.json");
        if (!fs2.existsSync(file)) return map;
    ...
    function createSessionsModule2(deps) {
      ...
      const HEAD_BYTES = 5e4;
      const originatorCache = /* @__PURE__ */ new Map();
      const topicCache = /* @__PURE__ */ new Map();
      function readFileHead(filePath, bytes) {
        const fd = fs2.openSync(filePath, "r");
        try {
          const buffer = Buffer.alloc(bytes);
          const read = fs2.readSync(fd, buffer, 0, bytes, 0);
      ...
      function getSessionOriginator(sessionId) {
        try {
          ...
          const mtimeMs = fs2.statSync(transcriptPath).mtimeMs;
    ...
    module2.exports = {
      ...
      CHANNEL_MAP,
      DEFAULT_CHANNEL_MAP,
      loadChannelMap: loadChannelMap2,
      parseSessionLabel
    };
  }
});
...
var require_cerebro = __commonJS({
  "src/cerebro.js"(exports2, module2) {
    ...
    function updateTopicStatus2(cerebroDir, topicId, newStatus) {
      ...
      const topicsRoot = path2.resolve(cerebroDir, "topics");
      const topicDir = path2.resolve(topicsRoot, topicId);
      if (!topicDir.startsWith(topicsRoot + path2.sep)) {
        return { error: "Invalid topic id", code: 400 };
      }
    ...
    function getRoutingStats2(skillsPath, statePath, hours = 24) {
      ...
      try {
        ...
        try {
          ...
          const cutoff = Date.now() - safeHours * 3600 * 1e3;
  ...
  "src/state.js"(exports2, module2) {
    ...
    function createStateModule2(deps) {
      ...
      function getCapacity() {
        ...
        try {
          const sessions2 = getRawSessions && getRawSessions() || [];
...
var { createSessionsModule, loadChannelMap } = require_sessions();
