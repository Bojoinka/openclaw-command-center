...
var require_actions = __commonJS({
  "src/actions.js"(exports2, module2) {
    var { listSessions, getSessionStatus } = require_gateway_api();
      ...
      try {
        switch (action) {
          ...
          case "sessions-list": {
            try {
              const sessions2 = await listSessions();
              const active = sessions2.filter((s) => s.status === "running").length;
              results.output = sessions2.length + " sessions (" + active + " active)";
            } catch (e) {
          ...
          case "health-check": {
            ...
            try {
              ...
              checks.push("Sessions: " + sessions2.length + " (" + active + " active)");
            } catch (e) {
          ...
          case "clear-stale-sessions": {
            ...
            try {
              const sessions2 = await listSessions();
              totalCount = sessions2.length;
              const nowMs = Date.now();
              staleCount = sessions2.filter((s) => {
                const updated = s.updatedAt || 0;
                return updated > 0 && nowMs - updated > 24 * 60 * 60 * 1e3;
              }).length;
            } catch (e) {
