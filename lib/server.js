      ...
      try {
        switch (action) {
          ...
          case "clear-stale-sessions": {
            // sessions --json can take 10-15s; use execFileSync directly with longer timeout
            let staleCount = 0;
            let totalCount = 0;
            try {
              const { execFileSync: efs2 } = require("child_process");
              const staleOutput = efs2("openclaw", ["sessions", "--json"], {
                encoding: "utf8",
                timeout: 20000,
                stdio: ["pipe", "pipe", "pipe"]
              });
              const jsonStart = staleOutput.search(/[\[{]/);
              if (jsonStart >= 0) {
                const data = JSON.parse(staleOutput.slice(jsonStart));
                const sessions = data.sessions || [];
                totalCount = sessions.length;
                staleCount = sessions.filter((s) => s.ageMs > 24 * 60 * 60 * 1e3).length;
              }
            } catch (e) {
              results.error = "Timed out querying sessions: " + e.message;
            }
