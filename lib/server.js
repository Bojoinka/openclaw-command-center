...
// src/gateway-api.js
var require_gateway_api = __commonJS({
  "src/gateway-api.js"(exports2, module2) {
    var http2 = require("http");
    var fs2 = require("fs");
    var path2 = require("path");
    var _cachedToken = null;
    var _tokenLoadedAt = 0;
    var TOKEN_CACHE_TTL = 6e4;
    function getGatewayToken() {
      ...
      try {
        const secretsPath = path2.join(
          process.env.HOME || "/home/odin",
          ".openclaw",
          "secrets.json"
        );
        const secrets = JSON.parse(fs2.readFileSync(secretsPath, "utf8"));
        _cachedToken = secrets.providers && secrets.providers.gatewayAuth && secrets.providers.gatewayAuth.token || "";
        _tokenLoadedAt = now;
      } catch (e) {
        _cachedToken = "";
      }
...
var require_sessions = __commonJS({
  "src/sessions.js"(exports2, module2) {
    ...
    var { listSessions: apiListSessions } = require_gateway_api();
    var CHANNEL_MAP = {
      c0aax7y80np: "#cc-meta",
    ...
    function createSessionsModule2(deps) {
      ...
      async function refreshSessionsCache() {
        ...
        try {
          let sessions2 = [];
          try {
            sessions2 = await apiListSessions();
          } catch (apiErr) {
            console.error("[Sessions Cache] API failed:", apiErr.message);
          }
          const filtered = sessions2.filter((s) => {
            if (s.kind === "group") return false;
            const key = s.key || "";
            if ((s.status === "done" || s.status === "failed") && (key.includes(":subagent:") || key.includes(":dashboard:"))) {
              return false;
            }
            return true;
          });
      ...
      function getSessions(options = {}) {
        ...
        let sessions2 = cached;
        const totalCount = sessions2.length;
        if (limit != null) {
      ...
      function getSessionDetail(sessionKey) {
        try {
          ...
          let sessionInfo = cachedSession ? {
            ...
            ageMs: (cachedSession.minutesAgo || 0) * 6e4,
...
var require_cron = __commonJS({
  "src/cron.js"(exports2, module2) {
    ...
    function getStateDbPath() {
      ...
      const profile = process.env.OPENCLAW_PROFILE;
      const openclawDir = profile ? path2.join(home, `.openclaw-${profile}`) : path2.join(home, ".openclaw");
      return path2.join(openclawDir, "state", "openclaw.sqlite");
    }
...
`;
    ...
    var cronCache = { data: null, timestamp: 0 };
    var CRON_CACHE_TTL_MS = 3e4;
    function getCronJobs2() {
      const now = Date.now();
    ...
    module2.exports = { getCronJobs: getCronJobs2, getStateDbPath };
  }
});
...
var require_llm_usage = __commonJS({
  "src/llm-usage.js"(exports2, module2) {
    var { listSessions } = require_gateway_api();
    var usageCache = { data: null, timestamp: 0, refreshing: false };
    var USAGE_CACHE_TTL_MS = 3e4;
    var BLENDED_RATES = {
      "claude-opus-4-6": 28,
      // $15 in + $75 out, blended ~$28/MTok
      "claude-sonnet-5": 9,
      // $3 in + $15 out, blended ~$9/MTok
      "claude-sonnet-4-6": 9,
      "claude-haiku-4-5": 1.75,
      // $0.80 in + $4 out, blended ~$1.75/MTok
      "gpt-5.4": 8,
      // $4 in + $16 out, blended ~$8/MTok
      "gpt-5.4-nano": 0.3,
      // $0.10 in + $0.40 out, blended ~$0.30/MTok
      "gpt-5.2": 8,
      "gpt-4.1": 4,
      "gpt-4o": 5,
      "gpt-4o-mini": 0.3,
      "grok-4.3": 8,
      // $3 in + $15 out, blended ~$8/MTok
      "default": 5
    };
    ...
    function buildBreakdown(sessions2) {
      const byModel = {};
      const byAgent = {};
      let totalTokens = 0;
      let totalCost = 0;
      ...
      for (const s of sessions2) {
        const model = (s.model || "unknown").replace("anthropic/", "").replace("openai/", "").replace("xai/", "");
      ...
      const modelBreakdown = Object.entries(byModel).map(([name, data]) => ({ name, ...data })).sort((a, b) => b.tokens - a.tokens);
      const agentBreakdown = Object.entries(byAgent).map(([name, data]) => ({ name, ...data })).sort((a, b) => b.tokens - a.tokens);
      return {
    ...
    function aggregateUsage(sessions2) {
      const todayMs = getTodayMidnight();
      const todaySessions = sessions2.filter((s) => {
        const updated = s.updatedAt || 0;
        return updated >= todayMs;
      });
      const allTime = buildBreakdown(sessions2);
    ...
    async function refreshLlmUsageAsync() {
      ...
      try {
        ...
        const result = aggregateUsage(sessions2);
        usageCache.data = result;
        usageCache.timestamp = Date.now();
        console.log(
          `[LLM Usage] Cache refreshed: ${sessions2.length} sessions, ${result.totalTokens.toLocaleString()} tokens, ${result.totalCostFormatted} (today: ${result.today.totalSessions} sessions, ${result.today.totalCostFormatted})`
        );
      } catch (e) {
...
var require_actions = __commonJS({
  "src/actions.js"(exports2, module2) {
    var { getSessionStatus } = require_gateway_api();
    ...
    async function executeAction2(action, deps) {
      const { PORT: PORT2, getSessionsCached } = deps;
      ...
      try {
        switch (action) {
          ...
          case "health-check": {
            ...
            checks.push("Dashboard: OK Running on port " + PORT2);
          ...
          case "clear-stale-sessions": {
            ...
            results.output = "Found " + staleCount + " stale sessions (>24h old) out of " + totalCount + " total.\nTo clean: openclaw sessions prune";
            results.success = true;
            break;
          }
        }
      ...
      function getCapacity() {
        ...
        try {
          ...
          for (const s of allSessions) {
            const ageMs = (s.minutesAgo || Infinity) * 6e4;
...
var server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/api/status") {
    ...
    execFile(pm2Bin, ["jlist"], { encoding: "utf8", timeout: 8e3 }, (err, stdout) => {
      if (err) {
        const next = pm2Candidates.find((b, i) => i > 0);
        if (next && next !== pm2Bin) {
          execFile(next, ["jlist"], { encoding: "utf8", timeout: 8e3 }, (err2, stdout2) => {
            if (err2) {
              res.writeHead(200, { "Content-Type": "application/json" });
    ...
    const { execFile } = require("child_process");
    execFile("tailscale", ["serve", "status", "--json"], { encoding: "utf8", timeout: 5e3 }, (err, stdout) => {
      if (err) {
        execFile("tailscale", ["serve", "status"], { encoding: "utf8", timeout: 5e3 }, (err2, stdout2) => {
          if (err2) {
            res.writeHead(200, { "Content-Type": "application/json" });
    ...
    execFile("node", [script, String(days)], { encoding: "utf8", timeout: 15e3 }, (err, stdout) => {
      if (err) {
        res.writeHead(500, { "Content-Type": "application/json" });
...
function parseTailscaleServeJson(json) {
  ...
  if (json.Web && typeof json.Web === "object") {
    const funnelPorts = new Set(Object.keys(json.AllowFunnel || {}).map((k) => k.split(":").pop()));
    for (const [hostPort, webCfg] of Object.entries(json.Web)) {
      ...
      for (const [path2, handlerCfg] of Object.entries(handlers)) {
        rows.push({
          proto: "https",
          port,
