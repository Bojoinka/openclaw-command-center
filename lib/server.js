...
var require_actions = __commonJS({
  "src/actions.js"(exports2, module2) {
    ...
    // Gateway HTTP API helper — ~0.2s vs ~12s CLI
    function gatewayInvoke(tool, args) {
      return new Promise((resolve, reject) => {
        ...
        let token = "";
        try {
          const sp = path2.join(process.env.HOME || "/home/odin", ".openclaw", "secrets.json");
          token = JSON.parse(fs2.readFileSync(sp, "utf8")).providers.gatewayAuth.token;
        } catch (e) {}
        const postData = JSON.stringify({ tool, args: args || {} });
        const req = http.request({
          ...
          res.on("end", () => {
            ...
            } catch (e) { reject(new Error("Bad JSON from gateway")); }
          });
        });
        req.on("error", reject);
    ...
    async function executeAction2(action, deps) {
      ...
      try {
        switch (action) {
          case "gateway-status": {
            // Gateway status via session_status tool
            try {
              const statusData = await gatewayInvoke("session_status", {});
              const lines = [];
          ...
          case "sessions-list": {
            try {
              const sessData = await gatewayInvoke("sessions_list", {});
              const ss = sessData.sessions || [];
              const active = ss.filter(s => s.status === "running").length;
              results.output = ss.length + " sessions (" + active + " active)";
            } catch (e) {
          ...
          case "health-check": {
            try {
              ...
              results.output = [
                "Gateway: OK Running",
                "Sessions: " + ss.length + " (" + active + " active)",
...
var server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/api/status") {
    ...
    // Real API spend from Anthropic + OpenAI admin APIs
    const { execFileSync: efs } = require("child_process");
    try {
      const days = parseInt(query.get("days") || "1", 10);
      const raw = efs("node", [
        "/home/odin/.openclaw/workspace/skills/api-costs/scripts/api-costs.js",
        String(days)
      ], { encoding: "utf8", timeout: 30000, stdio: ["pipe", "pipe", "pipe"] });
      const jsonStart = raw.search(/[\[{]/);
      const data = jsonStart >= 0 ? JSON.parse(raw.slice(jsonStart)) : { error: "no JSON" };
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify(data, null, 2));
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
